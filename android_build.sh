...
if [ "$NEWTEK_NDI_ENABLED" = "no" ]; then
  echo "Newtek NDI will be disabled for this build.  To enable it, set the NEWTEK_NDI_SDK environment variable."
fi
...
do
  ...
  ./zlib_build.sh $i $BASEDIR 0 || exit 1
